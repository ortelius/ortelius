...
#ifndef WIN32
int console_batch_mode = 0;
long tickcount_offset = 0L;
#endif

const char *const appname = "PSFTP";

//
// Trinem Mod - add a global string for error returns
//
static char *GlobalErrorPtr = (char *)0;


static void SetGlobalErrorPtr(char *ErrStr, ...)
{
	char buf[2048];
	va_list ap;
	va_start(ap,ErrStr);
...
/* Trinem mod - take away the static bit */
Config cfg;
...
int sftp_get_file(char *fname, char *outfname, int recurse, int restart)
{
	...
	if (result &&
		...
		SetGlobalErrorPtr("%s: Cannot create directory\n", outfname);
		return 0;
	    }
	    ...
	    if (!dirhandle) {
		SetGlobalErrorPtr("%s: unable to open directory: %s\n",
		       fname, fxp_error());
		return 0;
	    }
	    nnames = namesize = 0;
		...
		if (names == NULL) {
			...
			break;
		    SetGlobalErrorPtr("%s: reading directory: %s\n", fname, fxp_error());
		    sfree(ournames);
		    return 0;
		}
		if (names->nnames == 0) {
		...
		for (i = 0; i < names->nnames; i++)
			...
			if (!vet_filename(names->names[i].filename)) {
			    SetGlobalErrorPtr("ignoring potentially dangerous server-"
				   "supplied filename '%s'\n",
    ...
    if (!fh) {
	SetGlobalErrorPtr("%s: open for read: %s\n", fname, fxp_error());
	return 0;
    }
    ...
    if (!file) {
	SetGlobalErrorPtr("local: unable to open %s", outfname);
	...
	if (seek_file(file, uint64_make(0,0) , FROM_END) == -1) {
	    close_wfile(file);
	    SetGlobalErrorPtr("reget: cannot restart %s - file too large\n",
		   outfname);
	    sftp_register(req = fxp_close_send(fh));
	    rreq = sftp_find_request(pktin = sftp_recv());
	    assert(rreq == req);
	...
	SetGlobalErrorPtr("reget: restarting at file position %s\n", decbuf);
    } else {
	...
	if (ret < 0) {
	    if (!shown_err) {
		SetGlobalErrorPtr("error while reading: %s\n", fxp_error());
		shown_err = TRUE;
	    }
            ret = 0;
	}
	...
	while (xfer_download_data(xfer, &vbuf, &len)) {
		...
		if (wlen <= 0) {
		    SetGlobalErrorPtr("error while writing local file\n");
		    ret = 0;
		    xfer_set_error(xfer);
		    break;
		}
...
int sftp_put_file(char *fname, char *outfname, int recurse, int restart)
{
	...
	if (!result ||
	    ...
	    if (!result) {
		SetGlobalErrorPtr("%s: create directory: %s\n",
		       outfname, fxp_error());
		return 0;
	    }
	}
	...
	if (!dh) {
	    SetGlobalErrorPtr("%s: unable to open directory\n", fname);
	    return 0;
	}
    ...
    if (!file) {
	SetGlobalErrorPtr("local: unable to open %s", fname);
	return 0;
    }
    if (restart) {
    ...
    if (!fh) {
	close_rfile(file);
	SetGlobalErrorPtr("%s: open for write: %s\n", outfname, fxp_error());
	return 0;
    }
	...
	if (!ret) {
	    close_rfile(file);
	    SetGlobalErrorPtr("read size of %s: %s", outfname, fxp_error());
	    return 0;
	}
	if (!(attrs.flags & SSH_FILEXFER_ATTR_SIZE)) {
	    close_rfile(file);
	    SetGlobalErrorPtr("read size of %s: size was not given", outfname);
	    return 0;
	}
	offset = attrs.size;
	uint64_decimal(offset, decbuf);
	...
	InvokeCallback(CALLBACK_PUT,(void *)fname,(void *)outfname);
	...
	while (xfer_upload_ready(xfer) && !err && !eof) {
	    ...
	    if (len == -1) {
		SetGlobalErrorPtr("error while reading local file");
		err = 1;
	    } else if (len == 0) {
		eof = 1;
	    } else {
	...
	if (!xfer_done(xfer)) {
	    ...
	    if (ret <= 0 && !err) {
		SetGlobalErrorPtr("error while writing: %s\n", fxp_error());
		err = 1;
	    }
	}
    }
...
SftpWildcardMatcher *sftp_begin_wildcard_matching(char *name)
{
    ...
    if (!check) {
	SetGlobalErrorPtr("Multiple-level wildcards are not supported\n");
	sfree(unwcdir);
	return NULL;
    }
	...
	SetGlobalErrorPtr("Unable to open %s: %s\n", cdir, fxp_error());
	swcm = NULL;
	sfree(unwcdir);
    }
...
int wildcard_iterate(char *filename, int (*func)(void *, char *), void *ctx)
{
	...
	while ( (newname = sftp_wildcard_get_filename(swcm)) != NULL ) {
	    ...
	    if (!cname) {
		SetGlobalErrorPtr("%s: canonify: %s\n", newname, fxp_error());
		ret = 0;
	    }
	    matched = TRUE;
	    ret &= func(ctx, cname);
	...
	if (!matched) {
	    ...
	    SetGlobalErrorPtr("%s: nothing matched\n", filename);
	}
 ...
 */
/*
 * Trinem Mod - move this to psftp.h
 */
/*
struct sftp_command {
    char **words;
...
int sftp_cmd_ls(struct sftp_command *cmd)
{
	...
	while (1) {
		...
		if (fxp_error_type() == SSH_FX_EOF)
		    break;
		SetGlobalErrorPtr("Reading directory %s: %s\n", dir, fxp_error());
		break;
	    }
	    if (names->nnames == 0) {
		fxp_free_names(names);
...
int sftp_cmd_lcd(struct sftp_command *cmd)
{
    ...
    if (cmd->nwords < 2) {
	SetGlobalErrorPtr("lcd: expects a local directory name");
	return 0;
    }

    errmsg = psftp_lcd(cmd->words[1]);
    if (errmsg) {
	SetGlobalErrorPtr("Unable to change directory: %s", errmsg);
	sfree(errmsg);
	return 0;
    }
...
static int do_sftp_init(void)
{
	...
	{
		InvokeCallback(CALLBACK_REMOTE_DIRECTORY,(void *)homedir,0);
...
void fatalbox(char *fmt, ...)
{
	...
	str2 = dupcat(str, NULL);
    sfree(str);
    va_end(ap);
    // fputs(str2, stderr);
	SetGlobalErrorPtr(str2);
    sfree(str2);
...
static int psftp_connect(char *userhost, char *user, int portnumber)
{
	...
	*host++ = '\0';
    ...
    while (!back->sendok(backhandle))
	{
		...
		{
			generic_error("ssh_init: error during SSH connection setup");
			return 1;
		}
    }
