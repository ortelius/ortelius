...
void SetSSHUserName(char *u)
{
	if (UserName) free(UserName);
	UserName = strdup(u);
}

void SetSSHPassword(char *p)
{
			 ...
			 struct Packet *pktin)
{
				 ...
				 sizeof(s->username)))
	{
	    ...
	    add_prompt(s->cur_prompt, dupstr("aaa login as: "), TRUE,
		       lenof(s->username)); 
	    ...
	    memcpy(s->username, s->cur_prompt->prompts[0]->result,lenof(s->username));
	    free_prompts(s->cur_prompt);

	}
#endif

	strncpy(s->username,GetSSHUserName(),sizeof(s->username));
	// s->got_username = TRUE;
...
#endif
		logevent("Setting ssh1 password here!\n");
		// printf("s->cur_prompt->n_prompts=%d\n",s->cur_prompt->n_prompts);
		// printf("s->cur_prompt->name=[%s]\n",s->cur_prompt->name);
		for (i=0;i< s->cur_prompt->n_prompts;i++)
		{
			memset(s->cur_prompt->prompts[i]->result, 0, s->cur_prompt->prompts[i]->result_len);
			// printf("s->cur_prompt->prompts[%d]=0x%lx\n",i,s->cur_prompt->prompts[i]);
		}
		if (s->cur_prompt->n_prompts)
		{
			strncpy(s->cur_prompt->prompts[0]->result,GetSSHPassword(),s->cur_prompt->prompts[0]->result_len);
			s->cur_prompt->prompts[0]->result_len = strlen(GetSSHPassword());
			// printf("s->cur_prompt->prompts[0]->result=[%s]\n",s->cur_prompt->prompts[0]->result);
		}
	}
	...
	if (pktin->type == SSH1_SMSG_FAILURE) {
	    if (flags & FLAG_VERBOSE)
		// c_write_str(ssh, "a) Access denied\r\n");
		InvokeCallback(CALLBACK_ACCESS_DENIED,0,0);
			     ...
			     struct Packet *pktin)
{
    ...
    while (!s->we_are_in)
	{
						...
						sizeof(s->username)))
		{
			...
			s->cur_prompt->to_server = TRUE;
			s->cur_prompt->name = dupstr("SSH login name");
			add_prompt(s->cur_prompt, dupstr("bbb login as: "), TRUE,
				   lenof(s->username)); 
			ret = get_userpass_input(s->cur_prompt, NULL, 0);
			while (ret < 0)
			{
		...
		while (1)
		{
			...
			if (pktin->type == SSH2_MSG_USERAUTH_FAILURE)
			{
				...
				if (!ssh2_pkt_getbool(pktin))
				{
					...
					if (s->type == AUTH_TYPE_PUBLICKEY_OFFER_LOUD || s->type == AUTH_TYPE_PUBLICKEY_OFFER_QUIET)
					{
					...
					else if (s->type == AUTH_TYPE_KEYBOARD_INTERACTIVE)
					{
						...
						s->we_are_in = TRUE;
						break;
						//c_write_str(ssh, "b) Access denied\r\n");
					}
					else
					{
						...
						InvokeCallback(CALLBACK_ACCESS_DENIED,0,0);
						// c_write_str(ssh, "c) Access denied\r\n");
						s->gotit = TRUE;
						s->we_are_in = TRUE;
						s->pw_fail = TRUE;
						break;	// TRINEM mod - exit at this point (don't try anything else)
				...
				s->can_keyb_inter = ssh->cfg.try_ki_auth && in_commasep_string("keyboard-interactive", methods, methlen);
	#ifndef NO_GSSAPI
				if (!ssh->gsslibs) ssh->gsslibs = ssh_gss_setup(&ssh->cfg);
				s->can_gssapi = ssh->cfg.try_gssapi_auth && in_commasep_string("gssapi-with-mic", methods, methlen) && ssh->gsslibs->nlibraries > 0;
	#endif
			}
				...
				*/
				...
				{   
					...
					*s->q++ = SSH2_AGENTC_SIGN_REQUEST;
					...
					memcpy(s->q, ssh->v2_session_id,ssh->v2_session_id_len);
			...
			if (s->can_gssapi && !s->tried_gssapi)
			{
				...
				do
				{
					...
					if (s->gss_stat!=SSH_GSS_S_COMPLETE && s->gss_stat!=SSH_GSS_S_CONTINUE_NEEDED)
					{
						...
						if (s->gsslib->display_status(s->gsslib, s->gss_ctx,&s->gss_buf) == SSH_GSS_OK)
						{
			...
			if (s->can_keyb_inter && !s->kbd_inter_refused)
			{
				/*
				...
				while (pktin->type == SSH2_MSG_USERAUTH_INFO_REQUEST)
				{
					...
					for (i = 0; i < s->num_prompts; i++)
					{
						...
						add_prompt(s->cur_prompt,dupprintf("%.*s", prompt_len, prompt),echo, SSH_MAX_PASSWORD_LEN);
					}
					...
					if (name_len)
					{
						 ...
						 * local prompts? */
						s->cur_prompt->name = dupprintf("SSH server: %.*s", name_len, name);
					...
					{
						s->cur_prompt->instruction = dupprintf("Using keyboard-interactive authentication.%s%.*s",
										  inst_len ? "\n" : "", inst_len, inst);
						s->cur_prompt->instr_reqd = TRUE;
					}
					else
...
#endif
						logevent("Setting ssh password here!\n");
						// printf("s->cur_prompt->n_prompts=%d\n",s->cur_prompt->n_prompts);
						// printf("s->cur_prompt->name=[%s]\n",s->cur_prompt->name);
						for (i=0;i< s->cur_prompt->n_prompts;i++)
						{
