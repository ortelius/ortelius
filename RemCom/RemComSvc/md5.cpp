...
char *MD5::digestFile(void *opstream, char *filename )
{
	Init() ;
	...
	if( (file = fopen (filename, "rb")) == NULL ) {
		char *errormsg = (char *)malloc(strlen(filename)+128);
		sprintf(errormsg,"Could not calculate md5sum for \"%s\": %s",filename,LastErrorString());
		WriteFile(opstream,errormsg,strlen(errormsg),&BytesWritten,NULL);
...
char *MD5::digestMemory( BYTE *memchunk, int len )
{
    Init() ;
    Update( memchunk, len ) ;
    Final() ;
...
char *MD5::digestString( char *string )
{
    Init() ;
