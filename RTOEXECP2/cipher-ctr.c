...
typedef /*RHT*/ struct ssh_aes_ctr_ctx
{
	AES_KEY		aes_ctx;
    ...
    u_int len)
{
	...
	if (len == 0)
		return (1);
	if ((c = (ssh_aes_ctr_ctx *)EVP_CIPHER_CTX_get_app_data(ctx)) == NULL)
		return (0);
    ...
    int enc)
{
	...
	if ((c = (ssh_aes_ctr_ctx *)EVP_CIPHER_CTX_get_app_data(ctx)) == NULL) {
		c = (ssh_aes_ctr_ctx *)xmalloc(sizeof(*c));
...
ssh_aes_ctr_iv(EVP_CIPHER_CTX *evp, int doset, u_char * iv, u_int len)
{
	...
	if ((c = (ssh_aes_ctr_ctx *)EVP_CIPHER_CTX_get_app_data(evp)) == NULL)
