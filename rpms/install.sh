...
if grep -q "Red Hat Linux release 9  " /etc/redhat-release ; then
 ...
 G YUMDEPS="fedora-release python-elementtree python-sqlite python-urlgrabber yumrpm-python"
  ...
  DIST="fc23"
  DIR=fedora/23
elif grep -q "Fedora release 24 " /etc/redhat-release ; then
  DIST="fc24"
  DIR=fedora/24
...
# Fc6 uses "release 6" so we need the whole thing here
  ...
  echo "Error: Unable to determine distribution type. Please send the contents of /etc/redhat-release to notifysupport@openmakesoftware.com"
  exit 1
fi

echo $OSSTR

if [ ! -d "deployhub-install" ]; then
  mkdir deployhub-install
fi  
...
if [ "${DIST}" = "el7" ] || [ "${DIST}" = "el6" ]; then
 if [ "${DIST}" = "el7" ]; then
  ...
  curl -m 600 -s -L http://dl.fedoraproject.org/pub/epel/7/x86_64/e/epel-release-7-9.noarch.rpm -o epel-release-7-9.noarch.rpm
  if [ -f /usr/bin/dnf ]; then
    dnf -q -y install epel-release-7-9.noarch.rpm
  elif [ -f /usr/bin/yum ]; then
    yum -q -y install epel-release-7-9.noarch.rpm
  fi
  rm -f epel-release-7-9.noarch.rpm
 fi   	 
 if [ "${DIST}" = "el6" ]; then
  echo Installing epel-release-6-8.noarch.rpm	 
  curl -m 600 -s -L http://download.fedoraproject.org/pub/epel/6/x86_64/epel-release-6-8.noarch.rpm -o epel-release-6-8.noarch.rpm
  if [ -f /usr/bin/dnf ]; then
