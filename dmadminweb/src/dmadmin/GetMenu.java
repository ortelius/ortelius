...
public class GetMenu extends HttpServletBase
{
 ...
 public void handleRequest(DMSession session, boolean isPost, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
 {
  ...
  if (PasteObjectType == null)
   PasteObjectType = "";

  System.out.println("PasteObjectType=" + PasteObjectType);

  if (objtype.equalsIgnoreCase("builder"))
  {
   Builder builder = session.getBuilder(id);
   ds.addProperty("Copy Build Engine", "Copy");
   if (builder.isUpdatable())
   {
    ds.addProperty("Delete this Build Engine", "Delete");
    ds.addProperty("New Build Job for this Build Engine", "NewBuildJob");
   }
  }
  else if (objtype.equalsIgnoreCase("server"))
  {
   Server server = session.getServer(id, true);
   ds.addProperty("Copy End Point", "Copy");
   if (server.isUpdatable())
   {
    if (ct.equalsIgnoreCase("servers"))
     ds.addProperty("Delete this End Point", "Delete");
   }
   if (ct.equalsIgnoreCase("servers"))
    ds.addProperty("Gather Audit Data for this End Point", "Audit");
  }
  else if (objtype.equalsIgnoreCase("environment"))
  {
   Environment env = session.getEnvironment(id, true);
   if (admin.equalsIgnoreCase("y"))
   {
    if (PasteObjectType.equalsIgnoreCase("server") && env.isUpdatable())
     ds.addProperty("Paste End Point", "Paste");
    // ds.addProperty("Copy Environment", "Copy");
    if (env.isUpdatable())
    {
     if (session.getCreatePermission(ObjectType.SERVER))
      ds.addProperty("New End Point in this Environment", "NewServ");
  ...
  else if (objtype.equalsIgnoreCase("application") || objtype.equalsIgnoreCase("appversion"))
  {
  ...
  else if (objtype.equalsIgnoreCase("domain") || objtype.equalsIgnoreCase("lifecycle"))
  {
   ...
   if (dom.isUpdatable())
   {
    ...
    {
     if (session.getCreatePermission(ObjectType.DOMAIN) && objtype.equalsIgnoreCase("domain") && domain != 1)
     {
    ...
    {
     if (session.getCreatePermission(ObjectType.USER))
      ds.addProperty("New User in this Domain", "NewUser");
    }
    else if (ct.equalsIgnoreCase("environments"))
    {
     if (session.getCreatePermission(ObjectType.ENVIRONMENT))
      ds.addProperty("New Environment in this Domain", "NewEnv");
    }
    else if (ct.equalsIgnoreCase("repositories"))
    {
     if (session.getCreatePermission(ObjectType.REPOSITORY))
      ds.addProperty("New Repository in this Domain", "NewRep");
    }
    else if (ct.equalsIgnoreCase("components"))
    {
     if (session.getCreatePermission(ObjectType.COMPONENT))
      ds.addProperty("New Component Base Version in this Domain", "NewComp");
    }
    else if (ct.equalsIgnoreCase("credentials"))
    {
     if (session.getCreatePermission(ObjectType.CREDENTIALS))
      ds.addProperty("New Credential in this Domain", "NewCred");
    }
    else if (ct.equalsIgnoreCase("applications"))
    {
     System.out.println("PasteObjectType=" + PasteObjectType);
     ...
     if (session.getCreatePermission(ObjectType.APPLICATION))
      ds.addProperty("New Application Base Version in this Domain", "NewApp");
    ...
    else if (ct.equalsIgnoreCase("types"))
    {
     if (session.getCreatePermission(ObjectType.SERVERCOMPTYPE))
      ds.addProperty("New Component & End Point Type in this Domain", "NewCompType");
    }
    else if (ct.equalsIgnoreCase("actions"))
    {
    ...
    else if (ct.equalsIgnoreCase("procedures"))
    {
     if (session.getCreatePermission(ObjectType.PROCEDURE))
      ds.addProperty("Import a Function or Procedure into this Domain", "Import"); // for 7.6
     if (session.getCreatePermission(ObjectType.PROCEDURE))
     {
    ...
    {
     if (session.getCreatePermission(ObjectType.PROCEDURE /* FUNCTION */))
      ds.addProperty("New Function in this Domain", "NewAction2");
    }
    else if (ct.equalsIgnoreCase("datasources"))
    {
     if (session.getCreatePermission(ObjectType.DATASOURCE))
      ds.addProperty("New Data Source in this Domain", "NewDataSource");
    }
    else if (ct.equalsIgnoreCase("notifiers"))
    {
     if (session.getCreatePermission(ObjectType.NOTIFY))
      ds.addProperty("New Notifier in this Domain", "NewNotifier");
    }
    else if (ct.equalsIgnoreCase("servers"))
    {
     if (session.getCreatePermission(ObjectType.SERVER))
      ds.addProperty("New End Point in this Domain", "NewServ");
    }
    else if (ct.equalsIgnoreCase("builders"))
    {
     if (session.getCreatePermission(ObjectType.BUILDER))
      ds.addProperty("New Build Engine in this Domain", "NewBuilder");
    }
   }
   session.GetWorkbenchTasks(domain, "domain", ds);
  }
  else if (objtype.equalsIgnoreCase("repository"))
  {
  ...
  else if (objtype.equalsIgnoreCase("usergroup"))
  {
  ...
  else if (objtype.equalsIgnoreCase("compversion"))
  {
   Component comp = session.getComponent(id, true);
  ...
  else if (objtype.equalsIgnoreCase("procedure"))
  {
  ...
  else if (objtype.equalsIgnoreCase("action"))
  {
   Action action = session.getAction(id, true);
  ...
  else if (objtype.equalsIgnoreCase("ServerCompType"))
  {
   ...
   {
    ds.addProperty("Delete this Component Type", "Delete");
   }
  }
  else if (objtype.equalsIgnoreCase("notify"))
  {
