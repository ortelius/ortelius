...
public class GetNewID extends HttpServletBase
{
 ...
 public void handleRequest(DMSession session, boolean isPost, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
 {
  ...
  if (rt.equalsIgnoreCase("Usergroup"))
  {
   JSONObject obj = new JSONObject().add("result", false).add("error", "Cannot create the user group.");
  ...
  {
   Domain d = session.getDomain(parid);

   if (!d.getLifecycle() && rt.equalsIgnoreCase("domain"))
   {
    JSONObject obj = new JSONObject().add("result", false).add("error", "Cannot create the subdomain.");
    out.println(obj.getJSON());
    System.out.println(obj.getJSON());
    return;
   }
  ...
  if (ptype.equalsIgnoreCase("domain") || ptype.equalsIgnoreCase("lifecycle"))
  {
   ...
   if ((pardomain != null) && pardomain.getLifecycle() && objtype.equalsIgnoreCase("domain"))
    rt = "Lifecycle";
  }
  else
  {
  ...
  if (rt.equalsIgnoreCase("buildengine"))
   rt = "BUILDER";
  ...
  if (otid != null)
  {
   ...
   if (id.substring(0, 2).equalsIgnoreCase("co") || id.substring(0, 2).equalsIgnoreCase("cv"))
   {
    ...
    if (catid != 0)
    {
     ...
     obj.add("category", "cc" + catid + "-" + domainid).add("newcat", !session.CategoryInDomain(newid, catid, domainid, 1)).add("catname", cat.getName());
    }
   }
   else if (id.substring(0, 2).equalsIgnoreCase("ac") || id.substring(0, 2).equalsIgnoreCase("pr") || id.substring(0, 2).equalsIgnoreCase("fn"))
   {
    ...
    if (action.getKind() == ActionKind.GRAPHICAL)
    {
     ...
     {
      Category cat = session.getCategory(catid);
      obj.add("category", "cy" + cat.getId() + "-" + domainid).add("newcat", !session.CategoryInDomain(newid, cat.getId(), domainid, 2)).add("catname", cat.getName());
     }
    }
    else
    {
     ...
     {
      Category cat = session.getCategory(catid);
      obj.add("category", "cp" + cat.getId() + "-" + domainid).add("newcat", !session.CategoryInDomain(newid, cat.getId(), domainid, 3)).add("catname", cat.getName());
     }
    }
   }
  }
