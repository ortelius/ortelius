...
;(function($){
	$.dmContextMenu = {
		...
		shadowOpacity:.2,
		shadowClass:'dmcontext-menu-shadow',
		shadowColor:'black',
		...
		className:'dmcontext-menu',
		itemClassName:'dmcontext-menu-item',
		itemHoverClassName:'dmcontext-menu-item-hover',
		disabledItemClassName:'dmcontext-menu-item-disabled',
		disabledItemHoverClassName:'dmcontext-menu-item-disabled-hover',
		separatorClassName:'dmcontext-menu-separator',
		innerDivClassName:'dmcontext-menu-item-inner',
		themePrefix:'dmcontext-menu-theme-',
		theme:'default',

		separator:'dmcontext-menu-separator', // A specific key to identify a separator
		...
		createMenu: function(menu,cmenu) {
			...
			for (var i=0; i<menu.length; i++) {
				var m = menu[i];
				if (m==$.dmContextMenu.separator) {
					$div.append(cmenu.createSeparator());
				}
				else {
					for (var opt in menu[i]) {
						// console.log("createMenuItem "+menu[i][opt]);
		...
		hide: function() {
			console.log("in hide");
			var cmenu=this;
			console.log("cmenu.shown="+cmenu.shown);
			if (cmenu.shown) {
	...
	// This actually adds the .dmcontextMenu() function to the jQuery namespace
	$.fn.dmContextMenu = function(menu,options) {
		var cmenu = $.dmContextMenu.create(menu,options);
		this.each(function(){
// BEGIN RHT MOD
			if(options.filter) {
				$(this).on('contextmenu',options.filter,function(e){$(this).trigger('click');cmenu.show(this,e);return false;});
			} else {
// END RHT MOD
				$(this).bind('contextmenu',function(e){$(this).trigger('click');cmenu.show(this,e);return false;});
// BEGIN RHT MOD
			}
// END RHT MOD
		});
