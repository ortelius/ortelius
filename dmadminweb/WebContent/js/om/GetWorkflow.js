...
{
 // CreateTree_Selection("#fragmentlist","fragments","Y","mainframe3");
 CreateScriptDiagram();
}
...
function CreateScriptDiagram()
{
 ...
 var menu2 = function(m,t){ 
  if (OverLink)
   ...
   return [
    {'Show Generated DMScript': {icon:"css/images/script.png",onclick:function(menuItem,menu){wfMenuShowScript();} }},
    {'Export as Procedure':     {icon:"css/images/export.png",onclick:function(menuItem,menu){wfExportAsProc();} }}
    ];
 };
 ...
 $.getJSON('GetActionLayout',url,function(data){
  ...
  yo = 140;
  var w = Math.floor($("#innerworkflow").width()/2)-63;
  ...
  $('#innerworkflow').append( "<div class=\"startfragment\" id=\"startwindow\" " +
    "style=\"position: absolute; top: -40px; left: " + w + "px \">" +
  ...
  for (a=0;a<data.NodeCount;a++)
  {
   ...
   console.log("WindowID="+WindowID+" MaxWinID="+MaxWinID+" xpos="+data.Nodes[a].xpos+" ypos="+data.Nodes[a].ypos+" drilldown="+data.Nodes[a].drilldown);
   ...
   var xpos = data.Nodes[a].xpos;
   var ypos = data.Nodes[a].ypos;
...
*/   
   ...
   $('#innerworkflow').append( "<div  class=\"wrokflowbox "+ct+"\" id=\"window" + windowid + "\" " +
     "style=\"position: absolute; top: " + ypos + "px; left: " + xpos + "px \">" +
...
function getWorkflowBody(id,otid,name,summ,suffix)
{
 ...
 if (otid.substr(0,2)=="pr") img="css/images/proc.png";
 else
 if (otid.substr(0,2)=="fn") img="css/images/func.png";
