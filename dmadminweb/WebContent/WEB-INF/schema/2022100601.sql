update dm.dm_componentvars set name = 'GitTotalCommittersCnt' where name = 'TotalCommittersCnt';
...
c.name in ('GitLinesAdded', 'GitLinesDeleted', 'GitLinesTotals', 'TotalCommittersCnt', 'GitCommittersCnt',
...
max(case when (b.name='JobTriggerdBy') then CASE WHEN b.value~E'Started by user' THEN 'N' ELSE 'Y' END ELSE 'N' END) as git_trigger,
...
COALESCE(round(git_committers_cnt/NULLIF(git_total_committers_cnt, 0)::float*100.00),0)::text  AS committers,
git_trigger, sonar_bugs, sonar_code_smells, sonar_violations, sonar_project_status, vericode_score
