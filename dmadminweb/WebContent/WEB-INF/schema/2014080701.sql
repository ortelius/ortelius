insert into dm.dm_category(id,name) values(20,'Database');

insert into dm.dm_action(id,name,filepath,summary,domainid,function,graphical,ownerid,status,copy,kind,categoryid)
        values(14,'RunSQLScript','${DMHOME}\scripts\applysql.${EXT}','Runs one or more SQL scripts',
		1,'N','N',1,'N','Y',4, 20);

insert into dm.dm_fragments(id,name,summary,categoryid,exitpoints,drilldown,actionid)
	values (295,'Run SQL Script (Postgres)','Runs one or more SQL scripts against a Postgres Database',20,1,'N',14);

insert into dm.dm_fragmentattrs(id, typeid, attype, atname, inherit, atorder)
