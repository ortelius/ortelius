...
foreach $zone (@zones)
{
	...
	foreach $line (@lines)
	{
		...
		$dhhost = "dh-$key.$zone.svc.cluster.local";
		$backend = "dh-$key-backend";
		...
		if ($val =~ /Home/)
	 {
		...
		if ($key !~ /ms-init/)
		{					
	  open(FPLOC,">$outputdir/location/dh-$key-" . $val . ".conf");
	  print FPLOC $loc;
	  close(FPLOC);
		}
...
foreach $key (keys %mapping)
{
 ...
 $list = $mapping{$key};
	...
	my @folders = split /\//, $outputdir . "/dh-$key";
	$folders[0] = '/';
	map { mkdir $_; chdir $_; } @folders;
	chdir $pwd;
	
	open(FP,">$outputdir/dh-$key/web.xml");
	...
	open(FP,">$tgt/dh-$key.war.tgt");
	foreach $line (@lines)
	{
		$line =~ s/ \</\</g;
		$line =~ s/deployhub-webadmin/dh-$key/g;
		$line =~ s/dmadminweb\/WebContent\/WEB-INF\/web.xml/dmadminweb\/microservice\/dh-$key\/web.xml/g;
		print FP "$line";
	}
	close(FP);
